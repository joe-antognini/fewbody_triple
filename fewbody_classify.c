...
int fb_classify(fb_hier_t *hier, double t, double tidaltol, double speedtol, fb_units_t units, fb_input_t params)
{
  ...
  fb_init_hier(hier);
  fb_dprintf("in classify: %g %g\n", hier->hier[hier->hi[3]].x[0], hier->hier[hier->hi[2]].x[0]);
  ...
  while (cont) {
    ...
    if (cont) {
      ...
      fb_dprintf("before upsync: %g %g\n", hier->hier[hier->hi[3]].x[0], hier->hier[hier->hi[2]].x[0]);
      fb_upsync(&(hier->hier[hier->hi[n]+hier->narr[n]]), t, params, units);
      fb_dprintf("after upsync: %g %g\n", hier->hier[hier->hi[3]].x[0], hier->hier[hier->hi[2]].x[0]);
      hier->obj[isave[0]] = &(hier->hier[hier->hi[n]+hier->narr[n]]);
      hier->narr[n]++;

      fb_dprintf("created new object: %g %g\n", hier->hier[hier->hi[3]].x[0], hier->hier[hier->hi[2]].x[0]);
  ...
  /* JMA 6-8-12  -- We are satisfied if the inner binary in the triple has
   * merged. */
  if (hier->nstar == 2) {
    fb_dprintf("fewbody: classify(): merger\n");
    return(1);
  }

  /* JMA 6-12-12 -- We are also satisfied if the triple has become unbound.
   * (This only happens if the objects get close enough such that the PN
   * terms diverge.
   */
  if (hier->nobj == 2) {
    fb_dprintf("fewbody: classify(): unbound, so merger\n");
    return(1);
  }
...
{
  int i;
  double C=2.8, ain, aout, qout, eout, Rpout, inc, l0[3], l1[3], Lout[3];
  double Lin[3], Lbin[3], f;
  double a2, fquad;
  ...
  for (i=0; i<3; i++) {
    Lbin[i] = obj->obj[0]->m * l0[i] + obj->obj[1]->m * l1[i];
    Lbin[i] = Lbin[i];
  }

  /* JMA 06-08-12 -- The mistake was here!!!  LOLOLOLOL!  It's been fixed
   * now though. */
  for (i=0; i<3; i++) {
